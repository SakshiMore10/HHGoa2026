...
titleLogoImage.src = 'title-logo.png';
...
function switchMode(mode) {
  ...
  const idFields = ['name', 'phone', 'role', 'email', 'stack'];
  const isPfp = (mode === 'pfp-frame');

  idFields.forEach(f => {
    const group = document.getElementById(`group-${f}`);
    if (group) group.style.display = isPfp ? 'none' : 'flex';
  });

  const teamGroup = document.getElementById('group-team');
  if (teamGroup) teamGroup.style.display = isPfp ? 'flex' : 'none';
...
function renderBeachBadgeVerticalTemplate() {
  ...
  if (titleLogoImage && titleLogoImage.complete && titleLogoImage.naturalWidth !== 0) {
    ...
    ctx.shadowBlur = 10;
    ctx.drawImage(titleLogoImage, 500 - titleW / 2, 98, titleW, titleH);
    ctx.restore();
  } else {
  ...
  if (state.role.trim()) {
    ...
    ctx.shadowBlur = 10;
    ctx.textAlign = 'center';
    ctx.fillText(state.role.trim(), 500, 780);
    ctx.shadowBlur = 0;
  }

  // Details Grid Contents (No container box frame - text rendered cleanly directly on background)
  const gridY = 825;
  const startFieldX = 65;

  const labelsX1 = startFieldX + 30;  // 95
  const valuesX1 = startFieldX + 160; // 225
  const labelsX2 = startFieldX + 380; // 445
  const valuesX2 = startFieldX + 470; // 535

  let currentY = gridY + 30;
  const lineGap = 65;

  // Row 1: ROLE & PHONE
  drawFieldKey(labelsX1, currentY, 'ROLE');
  drawFieldValue(valuesX1, currentY, state.role, 190);

  drawFieldKey(labelsX2, currentY, 'PHONE');
  drawFieldValue(valuesX2, currentY, state.phone, 130);

  // Row 2: EMAIL
  currentY += lineGap;
  drawFieldKey(labelsX1, currentY, 'EMAIL');
  drawFieldValue(valuesX1, currentY, state.email, 430);

  // Row 3: TECH STACK
  currentY += lineGap;
  drawFieldKey(labelsX1, currentY, 'STACK');
  drawFieldValue(valuesX1, currentY, state.stack, 430, '#FFE600');
  ...
  // Real Scannable QR Code centered at the bottom (cx = 500, cy = 1265)
  drawRealScannableQRCode(500, 1265, 185);
...
function renderBoardingPassHorizontalTemplate() {
  ...
  ctx.fillStyle = '#062B21';
  ctx.font = '900 28px Space Grotesk, monospace';
  ctx.textAlign = 'center';
  ...
  ctx.fillStyle = '#062B21';
  ctx.fillText('HACKER HOUSE ', 560, 110);

  ctx.font = 'bold 50px "Tiro Devanagari Hindi", serif';
  ctx.fillStyle = '#FF1493';
  ctx.fillText('गोवा', 970, 110);
  ...
  drawTicketLabel(startX2, stubY, 'PHONE');
  drawTicketValue(startX2, stubY + 32, state.phone);

  stubY += 105;
  drawTicketLabel(startX1, stubY, 'ROLE & STACK');
  const roleStackVal = [state.role, state.stack].filter(Boolean).join(' • ');
  drawTicketValue(startX1, stubY + 32, roleStackVal, 440);

  drawTicketLabel(startX2, stubY, 'EMAIL');
  drawTicketValue(startX2, stubY + 32, state.email, 440);
  ...
  if (typeof QRCode !== 'undefined') {
    try {
      ...
      if (qrCanvas) {
        ...
        ctx.shadowBlur = 14;
        roundRect(ctx, cx - size / 2 - 14, cy - size / 2 - 14, size + 28, size + 28, 16, true, false);
        ctx.shadowBlur = 0;
...
function drawTicketValue(x, y, text, maxW = 380, color = '#062B21') {
  ...
  ctx.textAlign = 'left';
  ctx.font = '800 26px Space Grotesk, monospace';
  ctx.fillStyle = color;
