...
cardBgImage.src = 'card-bg.png?v=3';
cardBgImage.onload = () => {
...
function renderBeachBadgeVerticalTemplate() {
  ...
  if (state.name.trim()) {
    ctx.textAlign = 'center';
    ctx.font = '900 62px "Sitka Text", "Sitka", "Lora", serif';
    ctx.fillStyle = '#FFFFFF';
    ctx.shadowColor = 'rgba(0,0,0,0.7)';
    ctx.shadowBlur = 12;
    ctx.fillText(state.name.trim().toUpperCase(), 500, 760);
    ctx.shadowBlur = 0;
  }

  // Role Subtitle
  if (state.role.trim()) {
    ctx.font = 'bold 34px "Sitka Text", "Sitka", "Lora", serif';
    ctx.fillStyle = '#FFE600';
    ctx.shadowColor = 'rgba(0,0,0,0.7)';
    ctx.shadowBlur = 10;
    ctx.textAlign = 'center';
    ctx.fillText(state.role.trim(), 500, 805);
    ctx.shadowBlur = 0;
  }
  ...
  // Real Scannable QR Code centered at the bottom (cx = 500, cy = 1315)
  drawRealScannableQRCode(500, 1315, 185);

  // #FrameInGoa High-Visibility Pill Frame Badge on the left side of the QR code
  ctx.save();
  const hashtagFrameX = 215;
  const hashtagFrameY = 1315;
  const hW = 280;
  const hH = 58;
